...
sepalPlt = sb.FacetGrid(dataset, hue="WDWE", size=6).map(plt.scatter, "PM2.5", "NO2")
plt.legend(loc='upper left');

parallel_coordinates(dataset.drop('', axis=1), "WDWE");
#Data setup
...
p = [WDWE[np.argmax(P[i, :])] for i in range(X_test.shape[0])]

print("Test Accuracy ", accuracy_score(y_test, p) * 100 , '%')

#Confusion Matrix
cfm = confusion_matrix(y_test, p, labels = WDWE)

sb.heatmap(cfm, annot = True, xticklabels = WDWE, yticklabels = WDWE);
